...
internal class BombTimer(Graphics graphics) : IDisposable
{
    ...
    protected virtual void FrameAction()
    {
        ...
        _currentTime = graphics.GameProcess.Process.Read<float>(_globalVars + Offsets.MnCurrentTickThisFrame);
        ...
        _defuseCountDown = graphics.GameProcess.Process.Read<float>(_plantedC4 + Offsets.MFlDefuseCountDown);
        _c4Blow = graphics.GameProcess.Process.Read<float>(_plantedC4 + Offsets.MFlC4Blow);
        _beingDefused = graphics.GameProcess.Process.Read<bool>(_plantedC4 + Offsets.MbBeingDefused);
        ...
        if (_isBombPlanted)
        {
            _bombSite = graphics.GameProcess.Process.Read<int>(_plantedC4 + Offsets.MnBombSite) == 1 ? "B" : "A";
        }

        _bombPlanted = _isBombPlanted ? $"Bomb is planted on site: {_bombSite}" : string.Empty;

        graphics.GameProcess.Process.Read<bool>(_plantedC4 + Offsets.MbBombDefused);
    }
